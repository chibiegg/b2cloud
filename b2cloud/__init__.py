...
def login(customer_code: str, customer_password: str, customer_cls_cocde="", login_user_id="", target="b2cloud") -> requests.Session:
    ...
    data = {"quickLoginCheckH": "", "BTN_NM": "LOGIN", "serviceType": "portal", "CSTMR_CD": customer_code, "CSTMR_CLS_CD": customer_cls_cocde, "CSTMR_PSWD": customer_password, "LOGIN_USER_ID": login_user_id, "quickLoginCheck": ""}
...
def put_history_delete(session: requests.Session, feed: dict) -> dict:
    ...
    for entry in feed["feed"]["entry"]:
        _entry = {"id": entry["id"], "link": entry["link"], "shipment": {"tracking_number": entry["shipment"]["tracking_number"]}}
